...
public class VifDecoder
{
        ...
        int texturePixelHeight)
    {
        ...
        log.LogInformation("Num Meshes=" + numMeshes);
        log.LogInformation("Total Num Chunks=" + totalNumChunks);
            ...
            switch (vifCommand)
            {
                ...
                default:
                    if ((vifCommand & 0x60) == 0x60)
                    {
                        ...
                        {
                            // v4-32
                            log.LogDebug("v4-32 data, " + numCommand + (numCommand == 1 ? " entry" : " entries") +
                                         ", addr=" + addr);
                            if (1 == numCommand)
                            {
                            ...
                            {
                                log.LogWarning("unknown number of gif commands.");
                            }
                        ...
                        {
                            // v4-16
                            log.LogDebug("v4-16 data, " + numCommand + (numCommand == 1 ? " entry" : " entries") +
                                         ", addr=" + addr);
                            ...
                            {
                                log.LogWarning("Unsupported tag");
                            }
