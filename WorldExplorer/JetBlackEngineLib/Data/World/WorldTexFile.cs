...
public class WorldTexFile
{
    ...
    public WriteableBitmap? Decode(int offset, int chunkStartOffset)
    {
        ...
        var palette = PalEntry.UnswizzlePalette(PalEntry.ReadPalette(FileData, palOffset, 16, 16));
        var huffValues = DecodeHuff(palOffset + 0xc00);
        ...
        WriteableBitmap image = new(width, height, 96, 96, PixelFormats.Bgra32, null);
        image.Lock();
        ...
        while (FileData[p] != 0xFF)
        {
            ...
            for (var xBlock = x0; xBlock <= x1; ++xBlock)
            {
                ...
                DecodeBlock(xBlock, yBlock, blockDataStart, palOffset + 0x400, image, palette, huffValues);
                p += 4;
            }
        }
    ...
    private void DecodeBlock(int xBlock, int yBlock, int blockDataStart, int table0Start, WriteableBitmap image,
        PalEntry[] palette, HuffVal[] huffValues)
    {
